...
interface PropsData {
  ...
  switchToUser: (id: number) => Promise<void>;
  setData: React.Dispatch<React.SetStateAction<UserInList[]>>;
  order: {
    label: ReactNode;
    key: string;
  }[];
...
export default function UsersTable(props: PropsData) {
  const handleClick = (id: number) => async (e: { key: string }) => {
    props.setData((prev: UserInList[]) =>
      prev.map((user) =>
        user.id === id
          ? { ...user, role: e.key === "admin" ? true : false }
          : user
      )
    );
    console.log(e.key);
  ...
  const columns: ColumnsType<TableData> = [
    {
      ...
      render: (text: string, record) => (
        <span onClick={() => props.switchToUser(record.key)}>{text}</span>
      ),
    },
    {
      title: "No. messages",
